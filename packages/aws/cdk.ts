 ...
 */
const dynamodbTable = new dynamodb.Table(stack, 'Table', {
...
output(stack, 'TableName', dynamodbTable.tableName);

const policyToQueryPrivateKey = new iam.ManagedPolicy(stack, 'PolicyToQueryPrivateKey');
policyToQueryPrivateKey.addStatements(new iam.PolicyStatement({
...
const policyToPutPrivateKey = new iam.ManagedPolicy(stack, 'PolicyToPutPrivateKey');
policyToPutPrivateKey.addStatements(new iam.PolicyStatement({
...
const roleRotateKey = new iam.Role(stack, 'RotateKeyRole', {
    ...
    managedPolicies: [
        ...basePoliciesForLambda,
        policyToQueryPrivateKey,
        policyToPutPrivateKey,
    ],
});
const lambdaFnRotateKey: lambda.IFunction = new lambda.Function(stack, 'RotateKey', {
    ...
    environment: {
        ...
        TableRegion: dynamodbTable.stack.region,
    },
});
